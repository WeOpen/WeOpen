...
func TestHTTPHandlerPaginatesPosts(t *testing.T) {
	t.Parallel()

	handler := NewHTTPHandler(NewService(NewMemoryRepository(), nil))
	for _, slug := range []string{"one", "two", "three"} {
		createBody := bytes.NewBufferString(`{"title":"` + slug + `","slug":"` + slug + `","status":"draft"}`)
		createReq := httptest.NewRequest(http.MethodPost, "/posts", createBody)
		createRec := httptest.NewRecorder()
		handler.ServeHTTP(createRec, createReq)
		if createRec.Code != http.StatusCreated {
	...
	listReq := httptest.NewRequest(http.MethodGet, "/posts?limit=2&offset=1", nil)
	listRec := httptest.NewRecorder()
	handler.ServeHTTP(listRec, listReq)
	...
	if len(body.Posts) != 2 {
		t.Fatalf("expected 2 paginated posts, got %+v", body.Posts)
	}
	if body.Pagination.Total != 3 || body.Pagination.Limit != 2 || body.Pagination.Offset != 1 || body.Pagination.HasMore {
		t.Fatalf("expected pagination metadata for last page, got %+v", body.Pagination)
	}
}
