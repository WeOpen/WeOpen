...
// ListPostsPage returns a page of blog posts with total count metadata.
func (s *Service) ListPostsPage(ctx context.Context, filter ListPostsFilter) (ListPostsPage, error) {
	posts, err := s.repository.List(ctx, filter)
	if err != nil {
		return ListPostsPage{}, err
	}
	...
	return ListPostsPage{
		...
		HasMore: filter.Limit > 0 && filter.Offset+len(posts) < total,
	}, nil
}
